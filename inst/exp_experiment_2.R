...
df <- read.csv("exp_experiment_2.csv")
...
generate_data <- function(n, theta, tau, p) {
    ...
    #candidate_data <- NULL
    #repeat {
    #    candidate_data <- data %>% md_cand_sampler()
    #    X <- md_decode_matrix(candidate_data,"x")
        # make sure that each column has at least one TRUE value
        #if (all(colSums(X) > 0))
        #    break
    #}
    #candidate_data
...
exp_experiment_2_gen <- function(
    csv_filename,
    R = 1000,
    p = .333,
    q = .25,
    sample_sizes = c(20, 25, 30, 35, 40, 45, 50),
    append = TRUE,
    use_aneal_start = TRUE,
    retrieve_first_failed = FALSE) {
    # 5-component series system
    ...
    for (i in 1:length(sample_sizes)) {
        ...
        repeat {
            ...
            CI_lwr[j, ] <- confint(theta.hat)[,1]
            CI_upr[j, ] <- confint(theta.hat)[,2]
...
bad.data <- exp_experiment_2_gen(
    csv_filename = "exp_experiment_2.csv",
    #R = 1000, p = .333, q = .25,
    append = TRUE,
    retrieve_first_failed = TRUE,
    use_aneal_start = TRUE)

ll.bad <- md_loglike_exp_series_C1_C2_C3(bad.data)
ll.grad.bad <- md_score_exp_series_C1_C2_C3(bad.data)
ll.fim.bad <- md_fim_exp_series_C1_C2_C3(bad.data)
ll.numgrad.bad <- function(x) {
    numDeriv::grad(ll.bad, x, method.args = list(r=6))
}
ll.numhess.bad <- function(x) {
    numDeriv::hessian(ll.bad, x, method.args = list(r=6))
}
...
start <- sim_anneal(par = theta, fn = ll.bad, control =
    list(fnscale = -1, maxit = 20000000L, trace = TRUE,
        t_init = 100, 1e-6, alpha = 0.995,
        it_per_temp = 100L,
        neigh = function(par, temp, ...) {
            tt <- min(temp, 1)
            par + rnorm(length(par), mean = 0, sd = tt)
        },
        proj = function(par, ...) {
            par[par < 0] <- 1e-6
            par
...
bad.theta <- optim(
    ...
    method="L-BFGS-B",
    lower = 1e-30,
    control = list(maxit=10000L, fnscale=-1))

# Compute eigenvalues
eigenvalues <- eigen(-ll.fim.bad(bad.theta$par))$values
...
custom_solver <- function(data, theta, extra_info = NULL, annealing = TRUE) {
    ...
    tryCatch({
        #start <- optim(par = theta, fn = ll, 
        #    method = "SANN",
        #    control = list(fnscale = -1, maxit = 10000))
        start <- list(par = theta)
        if (annealing) {
            #start <- sim_anneal(par = theta, fn = ll, control =
            #    list(fnscale = -1, maxit = 1000L, trace = FALSE,
            #         t_init = 20, .001, alpha = 0.8,
            #         it_per_temp = 10L,
            #         sup = function(x) all(x > 0)))

            start3 <- sim_anneal(par = theta, fn = ll, control =
                list(fnscale = -1, maxit = 5000000L, trace = TRUE,
                    t_init = 100, 1e-6, alpha = 0.995,
                    it_per_temp = 100L,
                    neigh = function(par, temp, ...) {
                        tt <- min(temp, 1)
