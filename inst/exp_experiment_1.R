...
df <- read.csv("exp_experiment_1.csv")
...
exp_experiment_1_gen <- function(
    csv_filename,
    R = 1000,
    p = .333,
    q = .25,
    sample_sizes = c(35, 40, 45, 50, 60, 70, 80, 90, 100, 250, 500),
    append = TRUE,
...
exp_experiment_1_gen(
    csv_filename = "exp_experiment_1.csv",
    #R = 1000, p = .333, q = .25,
    sample_sizes = 20,
    append = FALSE,
    use_aneal_start = TRUE)
...
custom_solver <- function(data, theta, extra_info = NULL, annealing = TRUE) {
    ...
    fim <- md_fim_exp_series_C1_C2_C3(data)
    theta.hat <- NULL
    start <- NULL
    m <- length(theta)
