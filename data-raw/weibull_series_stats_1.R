...
set.seed(1729345)
N <- 100000
theta <- c(1,    10.25, # scale=lambda, shape=kappa
           1.75, 9.75,
           2.0,  15,
           1.5,  17.5)
m <- 4

# weibull_series_stats_2 <- tibble(
#     n = numeric(),
#     mse = numeric(),
#     frob = numeric(),
...
for (n in seq(from=100,to=N,by=100))
{
    ...
    l2 <- md_loglike_weibull_series_C1_C2_C3_2(md)
    mle <- mle_newton_raphson(l=l,theta0=theta)
    mle2 <- mle_newton_raphson(l=l2,theta0=theta)
