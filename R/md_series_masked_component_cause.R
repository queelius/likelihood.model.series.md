...
#' 
#' In the likelihood model, we assume that C[i] | T[i] = t, K[i] = k satisfies
#' certain assumptions. If component cause of failure is known, then we can
#' sample from the conditional distribution of C[i] | T[i] = t, K[i] = k,
#' otherwise we can sample from the empirical distribution of C[i] | T[i] = t[i],
...
#' calling this function, e.g., df <- df[df$delta, ]. Otherwise, the
#' function will sample from the empirical distribution of C[i] conditioned on
...
#' @export
md_sample_candidates <- function(n, df, t, k, lifetime = "t", candset = "x", nbins = 10) {
  ...
  df[indices, ] %>% select(starts_with(candset)) %>% sample_n(n, replace = TRUE)
}
