...
```{r sim-anneal, warning=F, cache=TRUE}
library(stats)
theta0 <- rep(5, m)
theta.start <- stats::optim(theta0, fn = function(theta) ll(data, theta),
    method = "SANN",
...
m <- length(theta)
fig2.R <- 500 # replicates
fig2.theta.hats <- matrix(nrow = fig2.R, ncol = m)

for (i in 1:fig2.R)
{
    data <- generate_data(n = n, theta = theta, tau = tau, p = p)
    fig2.theta.hats[i,] <- point(
        md_mle_exp_series_C1_C2_C3(md = data, theta0 = theta))
}

fig2.plot1 <- ggplot(tibble(x = fig2.theta.hats[, 1],
                            y = fig2.theta.hats[, 2]), aes(x = x,y = y)) +
