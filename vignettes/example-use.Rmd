---
title: "Example of how to use series.system.estimation.masked.data package"
output:
  html_document:
    df_print: paged
    toc: true
vignette: "%\\VignetteIndexEntry{The series.system.estimation.masked.data package}
...
Conditions
==========
The candidate set conditions, $C_1$, $C_2$, and $C_3$ may not be clear from the
definitions.

Arguably the most important condition in our statistical model is given by
Condition $C_1$, imposes the constraint $\Pr\{K_i \in C_i\} = 1$.
In a real-world setting, this condition may be too strict.

Let $\v{x} \mapsto \lVert \v{x} \rVert $ be a norm, which is a formalization of
*length*.
There are many kinds of norms, be we will be primarily interested in the L1
norm, $\lVert \v{x} \rVert = \sum_{i=1}^n |x_i|$ where $\v{x} \in \mathbb{R}^n$.

We may use this norm to define the distance measure given by
$$
    d(\v{x},\v{y}) = \lVert \v{y} - \v{x} \rVert,
$$
which in our case, since $\v{x},\v{y} \in \mathbb{R}$ and we are using the L1 norm,
$d(x,y) = |x-y|$.

### Condition $C_1$
We are interested in simulating masked data with candidate sets that satisfy
Condition $C_1$,
$$
    \Pr\{j \in \mathcal{C}_i | K_i = j} = 1.
$$
Let $d_{C_1}$ be defined as
$$
    d_{C_1}(j) = d(\Pr\{j \in \mathcal{C}_i | K_i = j},1).
$$

Then, Condition $C_1$ may be seen to take the form of satisfying
$$
    d_{C_1}(j) \geq 1.
$$
This may be too strict in reality, so we may replace the above with a
*relaxation*, $C_1 : [0,1] \mapsto \{0,1\}$, defined as
$$
...
$$
    ...
    \Pr\{\mathcal{C}_i = \mathbb{C}_i | K_i = j',T_i = t_i\}.
$$
Let $d_{C_2}$ be defined as
$$
    d_{C_2}(j,j') = \bigl|\Pr\{\mathcal{C}_i = \mathbb{C}_i | K_i = j\} -
                     \Pr\{\mathcal{C}_i = \mathbb{C}_i | K_i = j'\}\bigr|
$$
where $j,j' \in \mathbb{C}_i$.

Then, Condition $C_2$ (when we assume Condition $C_1$) may be seen to take the
form
$$
    \max_{q,r} \{ d_{C_2}(q,r) : q,r \in c_i \} \leq 0.
$$
This may be too strict in reality, so we replace the above with a *relaxation*
$C_2 : [0,1] \mapsto \{0,1\}$, defined as
$$
...
$\Pr\{\mathcal{C}_i = \mathbb{C}_i | K_i = j, T_i=t_i\}$ is independent of
$\v\theta$.
...
but it may certainly depend on the realizations of $T_{i 1},\ldots,T_{i n}$
and other factors not explicitly in the statistical model we have described.
...
will generate masked data that may be more or lesss informative (e.g., the
same MLEs but with more or less MSE).}
...
lifetime with a failure rate $\theta_j$, e.g., $\theta_2 = 4$ is the failure
rate of the component indexed by $4$.
...
gamma <- rep(.5,m)
alpha <- rep(1,n)
md <- md %>% md_bernoulli_candidate_alpha_C1_C3(gamma,alpha)
print(md)
```
...
The function `md_mle_exp_series_C1_C2_C3` is a small wrapper for this function,
providing `mle_gradient_ascent` with the appropriate arguments.
We find $\hat{\v\theta}$ by running the following R code:
```{r}
mle <- md_mle_exp_series_C1_C2_C3(md,theta0=theta)
theta.hat <- point(mle)
summary(mle)
```

The function `md_mle_exp_series_C1_C2_C3` returns an `md_mle` object, which
...
We let `theta.hat` be given by the `point` method, which obtains the *best*
the MLE $\hat{\v\theta}$.
...
Let us compute estimates of the bias and mean squared error as a function of
sample size $n$:

```{r, fig.width=5,fig.height=5,echo=F}
Ns <- seq(100, nrow(md), 100)
mle.bias <- matrix(nrow=length(Ns),ncol=3)
mles.vcov <- list(length=length(Ns))
mses.vcov <- numeric(length=length(Ns))
mses <- numeric(length=length(Ns))
mles <- list(length=length(Ns))
thetas <- matrix(nrow=length(Ns),ncol=3)

mse.hat <- function(theta.hats,theta)
{
    n <- nrow(theta.hats)
    s <- 0
    for (i in 1:n)
        s <- s + sum((theta.hats[i,] - theta)^2)
    s/n
}

mles[[1]] <- md_mle_exp_series_C1_C2_C3(md[1:Ns[1],],theta)
thetas[1,] <- point(mles[[1]])
mles.vcov[[1]] <- vcov(mles[[1]])
mses.vcov[1] <- sum(diag(mles.vcov[[1]]))
mses[1] <- sum((thetas[1,]-theta)^2)
mle.bias[1,] <- thetas[1,] - theta
for (i in 2:length(Ns))
{
    mles[[i]] <- md_mle_exp_series_C1_C2_C3(md[1:Ns[i],],theta)
    mles.vcov[[i]] <- vcov(mles[[i]])
    mses.vcov[i] <- sum(diag(mles.vcov[[i]]))
    thetas[i,] <- point(mles[[i]])
    mses[i] <- mse.hat(thetas[1:i,],theta)
    mle.bias[i,] <- colMeans(thetas[1:i,]) - theta
}
...
```{r}
new.md <- tibble(t1=rexp(n=n,rate=theta[1]),
             t2=rexp(n=n,rate=theta[2]),
             t3=rexp(n=n,rate=theta[3])) %>%
    md_series_lifetime() %>%
    md_bernoulli_candidate_alpha_C1_C3(gamma=rep(.3,3),
                                       alpha=rep(1,n))
                                       
mle.new <- md_mle_exp_series_C1_C2_C3(new.md,theta)
```
...
for (i in 1:fig2.N)
{
    fig2.md <- tibble(t1=rexp(n=n,rate=theta[1]),
                      ...
                      t3=rexp(n=n,rate=theta[3])) %>%
    md_series_lifetime() %>%
    md_bernoulli_candidate_alpha_C1_C3(gamma=rep(.5,3),
                                       alpha=rep(1,n))
    fig2.theta.hats[i,] <- point(md_mle_exp_series_C1_C2_C3(fig2.md,theta0=theta))
}
...
fig2.mle <- md_mle_exp_series_C1_C2_C3(md,theta)
fig2.theta.hat.asym <- rmvnorm(fig2.N,theta,vcov(fig2.mle))
...
```{r,eval=F,echo=F}
library(plotly)
plot_ly(x=mles[,1],y=mles[,2],z=mles[,3],
        type="scatter3d",mode="markers",size=.05) %>%
    plotly::layout(
        title="test",
...
the component failure rates are constant (i.e., not a function of $t$).
...
multivariate normal
$$
    \hat{k}_j(\v\theta) \sim \mathcal{N}(f_{K_i|T_i}(j|t_i,\v\theta), \v \Sigma_i).
$$
