...
vignette: >
  %\VignetteIndexEntry{The series.system.estimation.masked.data package}
...
```{r}
print(exp_series_data_1,drop_latent=T,pprint=T)
```

You can get help on any object in `series.system.estimation.masked.data` using the built-in help.
...
The following log-likelihood constructor, `md_loglike_exp_series_reg_cand`, is implemented using minimally sufficient
...
```{r,eval=F}
loglik <- md_loglike_exp_series_reg_cand(md)
```
...
```{r,eval=F}
mle <- md_mle_exp_series_reg_cand(md) 
theta.hat <- point(mle)
...
We see that $\hat\theta = (`r theta.hat <- c(1,1,1); round(theta.hat,digits=3)`)$.
...
```{r,message=F,eval=F}
prof <- function(theta1) { loglik(c(theta1,theta.hat[2],theta.hat[3])) }
...
We consider a larger and more complicated data set for this next example, `exp_series_data_1`.
Type `?exp_series_data_1` to learn more about it.
...
## Relaxing conditions
In real-world data sets, the conditions we assume may not be entirely realistic.
We may relax these conditions in order to see how well the MLE $\hat{\theta}$
performs under these ostensibly more realistic and more relaxed conditions.

The most important condition, $\Pr\{K_i = j|T_i=t_i\} = 1$, may be too stringent.
Instead, we can relax it by assuming only that $\Pr\{K_i = j|T_i=t_i\} = \alpha$
for some $\alpha$ in $(0,1)$.

The masking probability condition,
$$
\Pr\{C_i = c_i | K_i = j, T_i=t_i\} = \Pr\{C_i = c_i | K_i = j', T_i=t_i\}
$$
for any $j'$ in $c_i$ may also be relaxed by replacing the equality with
a distance measure, such as
$$
