...
```{r}
print(md_score_exp_series_C1_C2_C3)
...
for (i in N)
{
    ...
    data <- tibble(
        ...
        t3=rexp(N,theta[3])) %>%
        md_series_lifetime() %>%
        md_bernoulli_candidate_C1_C2_C3(m=m,p=function(n) rep(.5,n))

    loglik <- md_loglike_exp_series_C1_C2_C3(data)
    rates.hat <- mle_gradient_ascent(loglik,rates)
    loglik.gen <- function(md) md_loglike_exp_series_C1_C2_C3(md)
    rates.boot <- mle_samp_bs_loglike(rates.hat,loglik.gen,data,R)
    
    summary(rates.boot)
    
    mse(x=rates.boot,par=rates)
    bias(x=rates.boot,par=rates)
    point(rates.boot)
    vcov(rates.boot)
    confint(rates.boot)
}
