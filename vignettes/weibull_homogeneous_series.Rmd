...
We verify this identity numerically in Section \ref{exponential-identity}.
...
- **Shape estimation accuracy varies by regime.** The IFR regime ($k = 1.5$)
  produces the most precise shape estimates because increasing hazards generate
  more failures early, providing richer information. The DFR regime ($k = 0.7$)
  has the widest shape CIs because decreasing hazards spread failures over a
  longer horizon.

- **Scale estimation is robust across regimes.** Relative bias in the scale
  parameters is typically below 5% in all three regimes, with coverage near
  the nominal 95%. The scale parameters are well-identified regardless of the
  shape value.
...
The transformed Weibull scores match the exponential scores, confirming that
the two parameterizations yield identical inference at $k = 1$.
