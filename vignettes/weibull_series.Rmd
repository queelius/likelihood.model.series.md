...
- **Shape parameters are harder to estimate than scales.** The relative bias
  and RMSE for shape parameters $(k_1, k_2, k_3)$ are typically larger than for
  the corresponding scales $(\beta_1, \beta_2, \beta_3)$. This reflects the
  fundamental difficulty of estimating curvature from masked data: shape
  determines *how* the hazard changes, while scale determines *where* it sits.

- **DFR shapes ($k < 1$) are the hardest.** Component 1 ($k_1 = 0.8$) tends to
  show the largest relative estimation error among the shapes. When $k < 1$, the
  hazard diverges as $t \to 0$, making the likelihood surface more sensitive to
  the shape parameter near the boundary.

- **Coverage is near nominal.** The Wald confidence intervals achieve
  approximately 95% coverage across all parameters, validating the asymptotic
  normal approximation at this sample size.

- **The 2m-parameter model is identifiable.** Despite the doubling of parameters
  relative to the homogeneous model, all six parameters are consistently
  recovered. Identifiability relies on the candidate sets conveying information
  about which component caused the failure; without this (i.e., $p = 1$,
  maximum masking), the model would not be identifiable.
