...
2. `point(md_mle)` returns the point that maximizes the likelihood
   of the model.
...
6. `resid(md_mle)` computes the residuals of the fitted model.
   If `md_mle` is a `md_series_mle`, then uses
   covariate data like `x1`,...,`xm` and `t` to predict `t1`,...`tm`,
   i.e., it returns a matrix of residuals. It also returns the residual
   for `s <- min(t1,...,tm)`.
