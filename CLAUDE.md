...
- Component lifetimes may be masked (right-censored)
...
Each model implements S3 methods: `loglik()`, `score()`, `hess_loglik()`, `assumptions()`, `rdata()`
...
**Implementation note:** The exponential model computes score and Hessian analytically. The Weibull models compute score analytically but use `numDeriv::jacobian` on the score for the Hessian.
...
2. Apply `md_series_lifetime_right_censoring()` → adds `t`, `delta`
3. Apply `md_bernoulli_cand_c1_c2_c3()` → adds `q1, q2, ...`
4. Apply `md_cand_sampler()` → adds `x1, x2, ...`
...
- `md.tools`: `md_decode_matrix()`, `md_encode_matrix()`, `md_mark_latent()`
...
- Model constructors accept column name overrides: `lifetime`, `indicator`, `candset` params

## API Notes

**Unified Censoring API:** All models accept an `indicator` parameter (default: `"delta"`)
specifying the column for right-censoring status. For backwards compatibility, if this
column is absent, censoring is inferred from empty candidate sets (all FALSE).
