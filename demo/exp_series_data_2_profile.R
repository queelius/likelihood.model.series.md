...
mle <- point(est)
kloglik <- md_kloglike_exp_series_m0(exp_series_data_2)
profile2 <- function(x,y) { kloglik(c(x,y,mle[3],mle[4],mle[5])) }

# define the drawing grid
# create all possible combinations of x and y
data <- dplyr::tibble(x=seq(mle[1]-2,mle[1]+2,0.1),y=seq(mle[2]-2,mle[2]+2,0.1)) %>% purrr::cross_df()
n <- nrow(data)
data$z <- numeric(n)
for (i in 1:n)
...
profile1 <- function(x) { kloglik(c(x,mle[2],mle[3],mle[4],mle[5])) }
